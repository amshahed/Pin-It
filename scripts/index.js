	...
	$.ajax({
		...
		success: function(json){
			...
			for (var i=0; i<json.length; i++){
				...
				div += '<label onclick="seeImages(\''+json[i].email+'\')" class="owner">'+name+'</label>';
				div += '<div class="pull-right">';
				if (json[i].likes.includes(email))
					div += '<i style="color:red" class="fa fa-heart" onclick="likeImage(\''+json[i]._id+'\')"></i>&nbsp;&nbsp;';
				else
...
function likeImage(id){
	var email = localStorage.getItem('email');
	if (email==null){
		toastr.error('Please log in first', '', {timeOut:1300});
		return;
	}
	$.ajax({
		type: 'POST',
...
function seeImages(email){
	if (localStorage.getItem('email')==null){
		toastr.error('Please log in first', '', {timeOut: 1300});
		return;
	}
	localStorage.setItem('newmail', email);
	window.open('/seeimage', '_self');
}
